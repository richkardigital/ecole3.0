    ...
    try {
      ...
      if (exerciseFormat === 'DOCUMENT') {
        ...
        const payload = {
          ...
          type: exerciseType,
          isGraded,
          coefficient: isGraded ? coefficient : 0,
        ...
        if (exerciseId) {
          ...
          await api.post(`/chapters/${selectedChapterId}/exercises`, payload);
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              ...
              <p className="text-xs text-brand-text-muted mt-1 leading-relaxed">
                Créez une série de questions interactives (QCM, Vrai/Faux, Réponse courte) pour l'auto-évaluation immédiate des apprenants.
              </p>
            </button>
        ...
        <div className="bg-brand-card p-6 rounded-2xl border border-brand-border/80 shadow-sm space-y-5">
          <div className="flex items-center gap-2 pb-3 border-b border-brand-border/60">
            ...
            <h3 className="font-black text-lg text-brand-text">Affectation & Informations Pédagogiques</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                required
                className="w-full px-4 py-3 bg-brand-sidebar border border-brand-border/80 rounded-xl text-brand-text font-semibold text-sm focus:border-brand-accent outline-none transition cursor-pointer"
              >
                {chapters.map(c => (
                  <option key={c.id} value={c.id}>
                    {c.title} {c.term?.name ? `(${c.term.name})` : ''}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Ex: Exercices d'application sur les théorèmes"
                className="w-full px-4 py-3 bg-brand-sidebar border border-brand-border/80 rounded-xl text-brand-text font-semibold text-sm focus:border-brand-accent outline-none transition"
              />
            </div>
            ...
            {exerciseFormat === 'QUESTIONNAIRE' && (
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-brand-sidebar border border-brand-border/80 rounded-xl text-brand-text font-semibold text-sm focus:border-brand-accent outline-none transition cursor-pointer"
                >
                  <option value="QCM">QCM (Choix unique / Choix multiple)</option>
                  <option value="VRAI_FAUX">Vrai ou Faux</option>
                  <option value="TEXTE_LIBRE">Réponse libre / Rédactionnelle</option>
                </select>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <textarea
                ...
                placeholder="Consignes pédagogiques pour les élèves..."
                className="w-full px-4 py-3 bg-brand-sidebar border border-brand-border/80 rounded-xl text-brand-text text-sm focus:border-brand-accent outline-none transition resize-none"
              />
            </div>
                  ...
                  {(q.type === 'QCM' || q.type === 'VRAI_FAUX') && (
                    <div className="space-y-2 pt-2 border-t border-brand-border/50">
                      <div className="text-xs font-bold text-brand-text-muted uppercase tracking-wider mb-2">
                        Options de réponse (Indiquez la bonne réponse)
                      </div>
