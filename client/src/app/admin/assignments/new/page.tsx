...
export default function NewGlobalAssignmentPage() {
  ...
  return (
    <div className="p-6 md:p-8 lg:p-10 max-w-5xl mx-auto space-y-8 animate-in fade-in zoom-in duration-500 pb-32">
      ...
      <PageHeader 
        title="Programmer une nouvelle Évaluation" 
        description="Créez un devoir ou examen global, avec fichiers joints et/ou questionnaires intégrés."
              ...
              {files.length > 0 ? (
                <div className="flex flex-wrap gap-3">
                  {files.map((file, index) => (
                    <div key={index} className="flex items-center gap-2 bg-brand-surface px-3 py-2 rounded-lg border border-brand-border">
                      <span className="text-sm font-medium text-brand-text max-w-[200px] truncate">{file.name}</span>
                      <button type="button" onClick={() => removeFile(index)} className="text-brand-text-muted hover:text-red-500">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
                ...
                <p className="text-sm text-brand-text-muted italic">Aucun fichier joint pour le moment.</p>
              )}
            </div>
            )}
                ...
                <div className="space-y-6">
                  {questions.map((question, index) => (
                    <div key={question.id} className="bg-white p-5 rounded-xl border border-brand-border shadow-sm relative group">
                      ...
                      {/* Render Options Array manually for simplicity since it's nested */}
...
// Sub-component for options to keep it clean
